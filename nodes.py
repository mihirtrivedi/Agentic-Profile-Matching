...
def multi_round_screening_node(state: AgentState):
    ...
    prompt = (
        ...
        f"CRITICAL EVALUATION CRITERIA (HOLISTIC HIRING): "
        f"1. You must balance Experience and Must-Haves fairly. "
        f"2. If a candidate is highly experienced but misses a 'Must-Have', you MUST still mark them as 'Hire'. "
        f"3. If a candidate has slightly less experience but hits all the 'Must-Haves', you MUST also mark them as 'Hire'. "
        f"4. You MUST evaluate candidates relatively. Mark at least the top 3 or 4 candidates as 'Hire', selecting the best overall profiles. "
...
def intent_router(state: AgentState) -> str:
    ...
    prompt = (
        ...
        f"1. 'new_jd' : ONLY use this if the user provides a completely new Job Description, role requirements, or initiates a brand new search from scratch. It can be long or extremely short (e.g., 'Role: Python Developer. Must-have: SQL').\n"
